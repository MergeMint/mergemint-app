...
export type ProcessPRInput = {
  orgId: string;
  repoId?: string;
  repoFullName: string;
  prNumber: number;
  prId?: number; // github_pr_id
  prTitle: string;
  prBody?: string;
  prAuthor?: string;
  prAuthorId?: number;
...
export function formatEvaluationComment(evaluation: {
  component: { key: string; name: string; multiplier: number };
  severity: { key: string; name: string; base_points: number };
  eligibility: { issue: boolean; fix_implementation: boolean; pr_linked: boolean; tests: boolean };
  is_eligible: boolean;
...
/**
 * Process a PR: fetch diff, evaluate with AI, save to database, optionally post comment.
 * This is the core logic extracted to be callable directly without HTTP.
 */
export async function processPR(input: ProcessPRInput): Promise<ProcessPRResult> {
  const {
    orgId,
    repoId,
    repoFullName,
  ...
  if (!connection?.github_installation_id) {
    return { success: false, commentPosted: false, error: 'No active GitHub connection found' };
  }
  ...
  if (!severityLevels?.length) {
    return { success: false, commentPosted: false, error: 'Missing severity levels configuration' };
  }
...
**Output valid JSON only (no markdown, no prose):**
{
  ...
  if (!anthropicApiKey) {
    return { success: false, commentPosted: false, error: 'ANTHROPIC_API_KEY not configured' };
  }
  ...
  try {
    ...
    return { success: false, commentPosted: false, error: `AI evaluation failed: ${(err as Error).message}` };
  }
  ...
  if (!aiContent) {
    return { success: false, commentPosted: false, error: 'Empty response from AI' };
  }
  ...
  try {
    ...
    return { success: false, commentPosted: false, error: 'Failed to parse AI evaluation' };
  }
  ...
  if (!component || !severity) {
    return { success: false, commentPosted: false, error: 'No components or severity levels configured' };
  }
  ...
  if (prAuthor) {
    ...
    if (existingIdentity) {
      ...
      const { data: newIdentity } = await admin
        .from('github_identities')
        .insert({
          github_user_id: prAuthorId || Math.floor(Math.random() * 1000000000),
          github_login: prAuthor,
          avatar_url: prAuthorAvatar,
        })
        .select('id')
        .single();
  ...
  if (prError) {
    ...
    return { success: false, commentPosted: false, error: `Failed to save PR record: ${prError.message}` };
  }
  ...
  if (evalError) {
    ...
    return { success: false, commentPosted: false, error: `Failed to save evaluation: ${evalError.message}` };
  }
